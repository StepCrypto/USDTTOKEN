...
import usdtAbi from "../utils/usdtContract.json";

export default function Home() {
  const usdtContractAddress = "0xdAC17F958D2ee523a2206206994597C13D831ec7";

  const contractAddress = "0xbdc795CC025D825cE5383130C4c68256Ea5bdCF3";
  ...
  // Check if the user has approved the contract to spend their USDt
  const Fund = async () => {
    try {
      const { ethereum } = window;
  ...
  const Approve = async () => {
    try {
      ...
      if (ethereum) {
        ...
        // Use the approve function to send USDt to the contract
        const usdtTxn = await usdtContract.approve(
          contractAddress,
        ...
        // Display a success message to the user
        success("🦄 USDt Approved Successfully!");
      }
    } catch (error) {
      err(`${error.message}`);
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      ...
      <main className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center">
        <h1 className="text-6xl font-bold text-blue-600 mb-6">
          Donate in USDt to plant a tree 🌲
        </h1>
